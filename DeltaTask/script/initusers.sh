...
for user in $(yq ".users[] | .username" $YAML_DIR); do useradd -m -d /home/users/$user $user ; done
for admin in $(yq ".admins[] | .username" $YAML_DIR); do 
useradd -m -d /home/admins/$admin $admin ;done
for mod in $(yq ".mods[] | .username" $YAML_DIR); do 
useradd -m -d /home/mods/$mod $mod ;done
for author in $(yq ".authors[] | .username" $YAML_DIR);do 
    useradd -m -d /home/authors/$author $author;done
...
for author_d in $(yq '.authors[] | .username ' "$YAML_DIR" | sed 's/"//g');do
    ...
    chown $author_d /home/authors/$author_d/public
done 

for user_d in $(yq '.users[] | .username' "$YAML_DIR" |sed 's/"//g');do
    mkdir -p /home/users/$user_d/all_blogs
    chown $user_d /home/users/$user_d/all_blogs

    for auth_sym  in $(yq '.authors[] | .username' "$YAML_DIR" | sed 's/"//g');do
        ln -s /home/authors/$auth_sym/public /home/users/$user_d/all_blogs/$auth_sym
        done
done
#
