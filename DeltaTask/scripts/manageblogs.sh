...
EOF
    ...
    if [[ "$(yq '.categories' "$blogs_data_file")" == "null" ]]; then
        echo "Adding missing categories to $blogs_data_file"
        yq -i '
          .categories = {
            "1": "Sports",
            "2": "Cinema",
...
new() {
    ...
    if ! yq ".blogs[] | select(.file_name == \"$blogname\")" "$blogs_data_file" > /dev/null; then
...
if [[ -n "$mysql_command" ]]; then
    echo "$mysql_comment"
    echo "$mysql_command" | mysql -h db -uroot -pkali blogdb && echo "SUCCESS" || echo "FAILED"
fi
